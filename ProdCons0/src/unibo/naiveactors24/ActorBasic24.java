...
public abstract class ActorBasic24 implements IActor24 {
    ...
    //Struttura dati per salvare gli osservatori dell'attore:
    protected Set<ActorBasic24> obs = new HashSet<>();
...
//    }
    ...
    protected synchronized void reply( IApplMessage request, IApplMessage msg  )   {
    	sendReply(request,msg);
         if( ! msg.isReply() )  
            CommUtils.outyellow("		ActorBasic24 | warning: reply for non-reply");
         else {			//Modifica implementazione observer
    ...
    protected void sendMsg( IApplMessage msg, ActorContext24 ctx  )   {
        try { 
            ...
            if (dest != null) {
                ...
                //Nel seguente esempio di prova, per diventare osservatori di un attore è sufficiente inviare
                // un messaggio e l'attore destinatario registrerà il mittente come un suo obs.
                if(msg.msgSender().equals("logger")) {
                    dest.obs.add(this);
                }
            } 
            else {                
    ...
    protected void updateResource(String name) {
    	Iterator<ActorBasic24> iterator = obs.iterator();
        while (iterator.hasNext()) {
        	ActorBasic24 osservatore = iterator.next();
     		IApplMessage msg = CommUtils.buildDispatch( name, "update", "msg" , osservatore.getName());
     		forward(msg);
//     		CommUtils.outcyan("ActorBasic24 | Invio update: "+ msg+ " to: "+osservatore.getName());
        }
